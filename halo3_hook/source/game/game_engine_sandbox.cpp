...
static void __fastcall c_sandbox_engine__dispose_from_old_map(void *engine, void*);
static decltype(c_sandbox_engine__dispose_from_old_map) *c_sandbox_engine__dispose_from_old_map__original = nullptr;
...
{
	DetourDetach((PVOID *)&game_engine_sandbox_start_editing_object__original, game_engine_sandbox_start_editing_object);
	DetourDetach((PVOID *)&c_sandbox_engine__update__original, c_sandbox_engine__update);
	DetourDetach((PVOID *)&c_sandbox_engine__dispose_from_old_map__original, c_sandbox_engine__dispose_from_old_map);
...
static void __fastcall c_sandbox_engine__dispose_from_old_map(void *engine, void *address)
{
	c_sandbox_engine__dispose_from_old_map__original(engine, address);
...
static void __fastcall game_engine_sandbox_start_editing_object(long player_index, long map_variant_index)
{
	...
	{
		g_history_manager.push(L"object grabbed", game_engine_get_runtime_map_variant(), [](void *data)
			{
				assert(data);
			});
	}
